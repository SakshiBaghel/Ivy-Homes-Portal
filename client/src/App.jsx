...
export default function App() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      while (hasMore) {
        ...
        const totalReported = dataObj.total || 0;
        if (currentOffset + items.length >= totalReported || items.length === 0) {
          hasMore = false;
        } else {
      ...
      // ✅ FIX: item.bhk ki jagah item.bedroom use kiya kyunki dataset me bedroom field hai
      const fetchedBhks = uniqueItems.map(item => Number(item.bhk || item.bedroom)).filter(n => !isNaN(n) && n > 0);
  ...
  const applyFilters = (sourceData, customParams = {}) => {
    ...
    // ✅ FIX: item.bhk ki jagah item.bhk || item.bedroom check kiya taaki undefined na ho
    if (activeBhk) {
      filtered = filtered.filter(item => Number(item.bhk || item.bedroom) === Number(activeBhk));
    }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === 'analytics' && (
          <div>
            ...
            <AnalyticsCharts listings={allListings} />
          </div>
        )}
