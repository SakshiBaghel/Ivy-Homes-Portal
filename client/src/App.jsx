...
// //       // Bulk Fetch for dynamic filters
...
// //           // // BHKs
// //           // const uniqueBhks = [...new Set(allItems.map(item => Number(item.bhk)).filter(n => !isNaN(n) && n > 0))];
// //           // setBhkList(uniqueBhks.sort((a, b) => a - b));

// //           // BHKs
// //           const fetchedBhks = allItems.map(item => Number(item.bhk)).filter(n => !isNaN(n) && n > 0);
// //           const combinedBhks = [...new Set([...fetchedBhks, 1, 2, 3, 4, 5, 6, 7, 8])]; 
// //           setBhkList(combinedBhks.sort((a, b) => a - b));

// //           // Prices
// //           const prices = allItems.map(item => Number(item.price)).filter(p => !isNaN(p) && p > 0);
// //           const dynamicPrices = [];
// //           if (prices.some(p => p < 5000000)) dynamicPrices.push({ label: 'Under ₹50 Lakh', value: '0-5000000' });
// //           if (prices.some(p => p >= 5000000 && p < 10000000)) dynamicPrices.push({ label: '₹50L - ₹1 Crore', value: '5000000-10000000' });
// //           if (prices.some(p => p >= 10000000 && p < 20000000)) dynamicPrices.push({ label: '₹1 Crore - ₹2 Crore', value: '10000000-20000000' });
// //           if (prices.some(p => p >= 20000000 && p < 50000000)) dynamicPrices.push({ label: '₹2 Crore - ₹5 Crore', value: '20000000-50000000' });
// //           if (prices.some(p => p >= 50000000)) dynamicPrices.push({ label: 'Above ₹5 Crore', value: '50000000-9999999999' });
...
// //       const isClientFiltered = activeSearch || minPrice || maxPrice;

// //       const params = {
// //         page: isClientFiltered ? 1 : activePage, 
// //         limit: isClientFiltered ? 100 : 12,      
// //         ...(activeLoc && { locality: activeLoc }),
// //         ...(activeBhk && { bhk: activeBhk }),
// //         ...(activeFurn && { furnishing: activeFurn.toLowerCase() })
// //       };
...
// //       if (minPrice !== undefined) results = results.filter((item) => Number(item.price) >= minPrice);
// //       if (maxPrice !== undefined) results = results.filter((item) => Number(item.price) <= maxPrice);

// //       if (isClientFiltered) {
// //         const startIndex = (activePage - 1) * 12;
// //         const paginatedResults = results.slice(startIndex, startIndex + 12);
// //         setListings(paginatedResults);
// //         setTotalPages(Math.ceil(results.length / 12) || 1);
// //       } else {
// //         setListings(results);
// //         const hasNextPage = results.length === 12;
// //         setTotalPages(res.data?.total_pages || res.data?.totalPages || (hasNextPage ? activePage + 1 : activePage));
// //       }
...
//   //   setRentalsLoading(true);
//   //   try {
//   //     // 1. Projects Fetch (High limit ke sath)
//   //     const projRes = await fetchProjects({ limit: 1000 });
//   //     setProjects(projRes.data?.results || projRes.data?.data || (Array.isArray(projRes.data) ? projRes.data : []));

//   //     // 2. Rentals Fetch (Loop lagakar saara 1300+ data lana)
//   //     let allRentalsData = [];
//   //     let currentOffset = 0;
//   //     let hasMore = true;

//   //     while (hasMore) {
//   //       // limit 100 rakhi hai taaki requests kam lagain
//   //       const rentRes = await fetchRentals({ limit: 100, offset: currentOffset });
...
//   //       // Agar API ne 'has_more: false' bheja ya items 0 aaye, toh loop band karo
//   //       if (dataObj.has_more === false || items.length === 0) {
//   //         hasMore = false;
//   //       } else {
//   //         currentOffset += 100; // Next batch ke liye offset 100 badha do
//   //       }
//   //     }
...
//     setRentalsLoading(true);
//     try {
//       // 1. Projects Fetch (Loop lagakar saara data lana)
//       let allProjectsData = [];
//       let projOffset = 0;
...
//       // 2. Rentals Fetch (Loop lagakar saara data lana)
//       let allRentalsData = [];
//       let rentOffset = 0;
...
//               {rentals.map((r) => (
//                 <RentalCard key={r.listing_id || r.id} rental={r} />
//               ))}
//             </div>
//           </section>
//         )}
...
export default function App() {
  ...
  //   setLoading(true);
  //   try {
  //     const firstRes = await fetchListings({ page: 1, limit: 100 });
  //     let allItems = firstRes.data?.results || firstRes.data || [];
  //     const totalPagesFromAPI = firstRes.data?.total_pages || firstRes.data?.totalPages || 1;

  //     if (totalPagesFromAPI > 1) {
  //       const fetchPromises = [];
  //       for (let i = 2; i <= totalPagesFromAPI; i++) {
  //         fetchPromises.push(fetchListings({ page: i, limit: 100 }));
  //       }
